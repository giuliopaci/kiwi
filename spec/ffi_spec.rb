...
describe WikiParser do
  ...
  describe "templates" do
    ...
    it "should handle simple template replacement" do
      @parser.html_from_string("{{{1}}}").should_not include "__{1_5381__}"
      @parser.html_from_string("{{{1}}}").should_not include "__1_5381__"
      @parser.html_from_string("{{{1}}}").should include "{{{1}}}"
    end
  end
end
